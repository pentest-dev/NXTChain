...
contract("Ingress contract", (accounts) => {
    ...
    it("Should not allow an unauthorized account to perform administration operations", async () => {
        ...
        // }
        await expect(nodeIngressContract.setContractAddress(RULES, nodeRulesContract.address, { from: accounts[1] }))
            .to.be.revertedWith('Not authorized to update contract registry');
        ...
        //     expect(err.reason).to.contain("Not authorized to update contract registry");
        // }
        await expect(nodeIngressContract.removeContract(ADMIN, { from: accounts[1] }))
            .to.be.revertedWith("Not authorized to update contract registry");
    });
