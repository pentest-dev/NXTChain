...
contract("Admin (admin management)", async accounts => {
  ...
  it("non admin cannot add another admin", async () => {
    ...
    // }
    await expect(adminContract.addAdmin(accounts[2], { from: accounts[1] }))
  ...
  it("non admin cannot remove another admin", async () => {
    ...
    //   expect(err.reason).to.contain("Sender not authorized");
    // }
    await expect(adminContract.removeAdmin(accounts[2], { from: accounts[1] }))
        .to.be.revertedWith('Sender not authorized');
  });
  ...
  it("admin cannot remove themselves", async () => {
    ...
    // }
    await expect(adminContract.removeAdmin(accounts[0], { from: accounts[0] }))
        .to.be.revertedWith("Cannot invoke method with own account as parameter");
  });
